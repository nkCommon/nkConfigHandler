...
TYPE_MAPPERS: dict[int, Callable[[str], Any]] = {
    ...
    8: parse_date,  # Date
    9: lambda x: {int(k): v for k, v in (item.split(":") for item in x.split(","))},  # Dictionary[int,str]
    10: lambda x: {k: v for k, v in (item.split(":") for item in x.split(","))},  # Dictionary[str,str]
}
